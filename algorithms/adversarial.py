...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def get_action(self, state: GameState) -> str | None:
        ...
        """
        self.nodes_evaluated = 1  # la raíz cuenta como estado procesado

        def value(node: GameState, agent_index: int, plies_left: int) -> float:
            ...
            successors = (
                value(node.generate_successor(agent_index, action), next_agent, plies_left - 1)
                for action in legal
            )
            return max(successors) if agent_index == 0 else min(successors)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def get_action(self, state: GameState) -> str | None:
        ...
        def value(node: GameState, agent_index: int, plies_left: int, alpha: float, beta: float) -> float:
          self.nodes_evaluated += 1
          ...
          if agent_index == 0:
              ...
              for action in legal_actions:
                  ...
                  v = max(v, value(successor, next_agent, plies_left - 1, alpha, beta))
                  if v >= beta:
                      return v
                  alpha = max(alpha, v)
              return v
              ...
              for action in legal_actions:
                  successor = node.generate_successor(agent_index, action)
                  v = min(v, value(successor, next_agent, plies_left - 1, alpha, beta))
                  if v <= alpha:
                      return v
                  beta = min(beta, v)
              return v
        ...
        for action in actions:
            ...
            action_value = value(successor, 1, self.depth - 1, alpha, beta)
